...
Notice that the model have clustered fault density data according to their variance (noise level). Consequently, the model identified the fault density values generated by the furnace with the noise level 0, and other processes, such as the coating or bath processes, with higher noise levels.
...
The model alerts the user when it is at a region where the data is different from the one observed in the training stage by eliminating the confidence bounds. In other words, when there is no yellow area, the user must not use the model's predictions for decision-making.
