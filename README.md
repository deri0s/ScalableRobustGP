...
The following figure shows the regression performance of the standard Distributed GP (red line) and the proposed DDPGP (green line) when estimating the function, $f(x) = sin(x)$, using data corrupted by two independent noise sources. Notice that the Distributed GP (DGP) is affected by the data corrupted with a noise source whose variance is "large". On the other hand, the DDPGP predictions successfully ignored observations corrupted with "large" noise levels.

<div align="right">Zoomed-in view</div>
<div class="row">
...
We show the capability of the proposed model to estimate relationships using data obtained from industrial processes. The folder `real applications` contains a case study in continuous manufacturing where we estimate the relationship between the furnace parameters and product quality from measurements generated by the furnace, noise and non-furnace processes.


## Process identification - Signal Processing - Anomaly Detection
The image below displays the number of faults per $10\text{m}^2$ at 20 minute intervals. We will refer to this particular unit as <em>fault density</em>. The fault density data is extracted from scanners positioned at the end of the production line and is usually used in continuous manufacturing to determine product quality.

<div align="center">
    <img src="figures/NSG/ISRA_scanner_trend.png" alt="Snow" style="width:80%">
</div>

Notice that the data shows high peaks, noise bursts, and slow and rapid trends.
...
Alternatively, we test the DDPGP <b>process identification</b> capabilities. The model <b>automatically</b> ignores data generated by non-furnace processes when estimating the relationship between the furnace parameters and the number of faults. The results are shown in the image below.

<div align="center">
    <img src="figures/NSG/NSG_clustering.png" alt="Snow" style="width:80%">
</div>

Notice that the model have clustered fault density data according to their variance (noise level). Consequently, the model identified the fault density data generated by the furnace with the noise level 0, and other processes, such as the coating or bath processes (including independent noise processes), with higher noise levels.

Manufacturing experts have identified that the fault density increases located at 2020-08-21 and 2020-08-24 were exclusively generated by the furnace process. As a result, we have used the region between 2020-08-16 and 2020-09-01 for model validation. 

## Product Quality Forecast
The DDPGP model can be used as a decision tool since it has the following characteristics:
...
- <b>Uncertainty Quantification:</b> Quantifies the confidence of its preditions using the estiated (furnace) process variance to form confidence bounds.
- <b>New data alerts:</b> The model alerts when operating on data whose features differ from the training data. In such scenarios, the model predictions are not surrounded by confidence bounds and must not be used to influence decisions.
...
The model has been trained with $N=18057$ data points and the predictions shown in the above video corresponds to approximate $N_{test} = 1000$ data points.

The model alerts the user when it is at a region where the data is different from the one observed in the training stage by eliminating the confidence bounds. In other words, when there is no yellow area, the user must not use the model's predictions for decision-making. The model becomes less confident about its predictions when the confidence bounds increase. Accordingly, the model is confident about its predictions when the confidence bounds are narrow.

Notice that the conditioned furnace faults were obtained using large pre-processing pipelines developed using manufacturing experts' knowledge. The processed data provides an additional estimate of the number of faults generated by the furnace. However, it should not be considered the definitive ground truth for the furnace process. Therefore, we cannot use the processed data to assess the model's accuracy.

### Comparisons with a Feed-Forward Neural Network
...
The NN disadvantage (for this industrial application) is that it cannot perform uncertainty quantification, limiting its decision tool capabilities.
