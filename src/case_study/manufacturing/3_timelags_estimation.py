...
for i in range(D):
    x = X_df[X_df.columns[i]].values

    # Process signal and add to matrix of inputs
    x = dpm.adjust_time_lag(x, shift=time_lags[i],
                            to_remove=max_lag)
    print('X: ', np.shape(X))
    print('X: ', np.shape(x))
    X[:, i] = x
...
print(y_df.columns)
# date_time = dpm.adjust_time_lag(y_df..values,
#                                 shift=0,
#                                 to_remove=max_lag)
# date_time = dpm.adjust_time_lag(y_df.date_time.values,
#                                 shift=0,
#                                 to_remove=max_lag)

# print('x-size : ', X.shape)
# print('y-val  : ', y.shape)
# print('dt in y: ', date_time.shape)
