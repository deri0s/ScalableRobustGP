...
"""
data_ratio = {'training': 0.80, 'eval': 0.20}
...
# Validation data
X_eval_np = X[end_train: N]
y_eval_nonstand = y_all_nonstand[end_train: N]

# --- Input Validation: Check if train/test splits are valid ---
if N_train <= 0 or len(X_eval_np) <= 0:
    raise ValueError(f"Training size ({N_train}) or Test size ({len(X_eval_np)}) is zero or negative. Check data splitting.")
if len(y_eval_nonstand) != len(X_eval_np):
     raise ValueError(f"Test features ({len(X_eval_np)}) and test targets ({len(y_eval_nonstand)}) lengths differ.")
...
y_eval_reshape = y_eval_nonstand.reshape(-1,1)
y_eval_stand = scaler.transform(y_eval_reshape)
...
X_eval = torch.tensor(X_eval_np, dtype=floating_point)
y_eval = torch.tensor(y_eval_stand, dtype=floating_point).squeeze()

X_all = torch.tensor(X, dtype=floating_point)
...
auto_trainer = GPTraining(gp0, X_train, y_train, X_eval, y_eval)
...
if removed_indices:
    ...
    X_eval_filtered = X_eval[:, relevant_indices] 
...
if gp_filtered is not gp0: # Only tune if we have a model to tune
    ...
    if removed_indices:
        auto_trainer_filtered = GPTraining(gp_filtered, X_train_filtered, y_train, X_eval_filtered, y_eval)
    ...
    # Ensure we're using the same scaler for all predictions
    print(f"Using scaler with mean: {scaler.mean_[0]:.6f}, scale: {scaler.scale_[0]:.6f}")
    ...
    print("=== DETAILED TUNED MODEL DEBUG ===")
    
    # Check if tuned model is actually different from original
    print(f"Original model noise: {gp_gs.likelihood.noise.item():.6f}")
    print(f"Tuned model noise: {tuned_gp.likelihood.noise.item():.6f}")
    print(f"Models are same object: {tuned_gp is gp_gs}")
    
    # Check training data that was used
    print(f"Training targets used - mean: {y_train.mean():.6f}, std: {y_train.std():.6f}")
    print(f"Training targets should be standardized (mean~0, std~1)")
    
    # DEBUG: Check raw predictions in detail
    raw_pred_mean = observed_pred_tuned.mean
    print(f"Raw tuned predictions stats:")
    print(f"  Mean: {raw_pred_mean.mean():.6f}")
    print(f"  Std: {raw_pred_mean.std():.6f}")
    print(f"  Min: {raw_pred_mean.min():.6f}")
    print(f"  Max: {raw_pred_mean.max():.6f}")
    print(f"  First 5 values: {raw_pred_mean[:5].tolist()}")
    
    # Compare with original model raw predictions
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        observed_pred_original = gp_gs.likelihood(gp_gs(X_all))
        
    print(f"Original model raw predictions stats:")
    print(f"  Mean: {observed_pred_original.mean.mean():.6f}")
    print(f"  Std: {observed_pred_original.mean.std():.6f}")
    print(f"  Min: {observed_pred_original.mean.min():.6f}")
    print(f"  Max: {observed_pred_original.mean.max():.6f}")
    print(f"  First 5 values: {observed_pred_original.mean[:5].tolist()}")
