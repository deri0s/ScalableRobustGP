...
for index in range(N_partitions):
    ...
    (mse_all, mse_train, mse_test, mu, lower, upper) = predict_and_eval(
        gp, likelihood, scaler, X, X_train, X_test
    )
    ...
    plt.title(
        f"Expert: {index}, K: {k_name}, MSE-train: {mse_train:.4f}, MSE-test: {mse_test:.4f}, MSE-all: {mse_all:.4f}"
    )
    ax.fill_between(date_time, lower, upper, alpha=0.3, color="coral", label="95% CI")
